...
module AsciidoctorBibtex
  ...
  #
  class CitationItem
    attr_reader :key, :locator, :pretext
  ...
  class CitationMacro 
    ...
    CITATION_LIST_TAIL = /(\s*,\s*#{CITATION_ITEM})*/
    CITATION_LIST = /(?:#{CITATION_ITEM}#{CITATION_LIST_TAIL})/
    CITATION_PRETEXT = /[^\[]*/
    # matches the full citation macro
    CITATION_MACRO = /(#{CITATION_TYPE}):(#{CITATION_PRETEXT})\[(#{CITATION_LIST})\]/

    # Given a line, return a list of CitationData instances
    # containing information on each set of citation information
    def CitationMacro.extract_macros line
      result = []
