...
- use in one of two ways:
    ...
    - asciidoctor extension provided for single-pass compilation of document with references
...
- 'asciidoc-bib' for transforming source text
- 'asciidoctor-bib' uses asciidoctor extension

asciidoc-bib depends on bibtex-ruby[http://github.com/inukshuk/bibtex-ruby] and
citeproc-ruby[http://github.com/inukshuk/citeproc-ruby].
...
The first is required if using asciidoc (the original implementation).
...
The second is for asciidoctor only, and uses its extension mechanism to 
provide a single pass compilation of your documentation, including references.
asciidoctor-bib works as a replacement for asciidoctor.
